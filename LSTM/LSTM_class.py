"""
This code is taken from https://github.com/klaudia-nazarko/nlg-text-generation.git
"""
import re
import numpy as np
import keras
...
class Sequences():
    ...
    def sequences_info(self):
        print('number of sequences of length %d: %d' %
              (self.max_len, len(self.sequences)))
...
class ModelPredict():
    ...
    def generate_sequence(self, k, append=False, temperature=1):
        ...
        return_tokens_ind = ' '.join(
            [self.ind2token[ind] for ind in return_tokens_ind])
